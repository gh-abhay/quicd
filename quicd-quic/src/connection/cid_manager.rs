...
impl ConnectionIdManager {
    ...
    pub fn new(
        ...
        active_cids.insert(0, ActiveConnectionId {
            sequence_number: 0,
    ...
    pub fn issue_new_cids(&mut self, count: usize) -> Result<Vec<NewConnectionIdData>> {
        ...
        for _ in 0..count {
            ...
            self.active_cids.insert(seq_num, ActiveConnectionId {
...
mod tests {
    ...
    fn test_generator() -> Box<dyn ConnectionIdGenerator> {
        Box::new(RandomConnectionIdGenerator::new([42u8; 32]))
    }
    ...
    fn test_cid_manager_creation() {
        let initial_cid = test_cid();
        let manager = ConnectionIdManager::new(test_generator(), initial_cid.clone(), 8);
        
        assert_eq!(manager.active_count(), 1);
        assert_eq!(manager.current_cid().unwrap().as_bytes(), initial_cid.as_bytes());
    }
    
    #[test]
    fn test_issue_new_cids() {
        let initial_cid = test_cid();
        let mut manager = ConnectionIdManager::new(test_generator(), initial_cid, 8);
        manager.set_peer_cid_limit(8); // Allow issuing more CIDs
        ...
        assert_eq!(manager.active_count(), 4); // Initial + 3 new
    }
    
    #[test]
    fn test_cid_limit_enforcement() {
        let initial_cid = test_cid();
        let mut manager = ConnectionIdManager::new(test_generator(), initial_cid, 8);
        manager.set_peer_cid_limit(3);
        
        // Can only issue 2 more (limit=3, have 1 initial)
    ...
    #[test]
    fn test_current_cid_selection() {
        let initial_cid = test_cid();
        let mut manager = ConnectionIdManager::new(test_generator(), initial_cid.clone(), 8);
        manager.set_peer_cid_limit(8); // Allow issuing more CIDs
        
        // Current should be initial (seq 0)
        assert_eq!(manager.current_cid().unwrap().as_bytes(), initial_cid.as_bytes());
        
        // Issue new CIDs
        let new_cids = manager.issue_new_cids(2).unwrap();
        
        // Current should be highest sequence (2)
        assert_eq!(
            manager.current_cid().unwrap().as_bytes(),
            new_cids[1].connection_id.as_bytes()
        );
    }
}
