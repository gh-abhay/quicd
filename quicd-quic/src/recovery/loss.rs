...
mod tests {
    ...
    fn create_detector() -> DefaultLossDetector {
        DefaultLossDetector::new(LossDetectionConfig::default())
    }

    #[test]
    fn test_loss_detector_creation() {
    ...
    fn test_loss_by_packet_threshold() {
        ...
        // May or may not include 7 depending on exact threshold logic
        assert!(!lost.contains(&8)); // Within threshold
        assert!(!lost.contains(&9));
