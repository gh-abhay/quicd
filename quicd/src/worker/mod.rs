...
impl NetworkWorker {
    ...
    pub fn run(mut self) -> Result<()> {
        ...
        loop {
            ...
            if should_check_timeouts {
                match quic_manager.handle_timeouts() {
                    Ok(timeout_packets) => {
                        ...
                        let coalesced = coalesce_packets(timeout_packets, self.config.max_coalesced_size);
            ...
            let coalesced_egress = coalesce_packets(egress_packets, self.config.max_coalesced_size);
            ...
            // ═══════════════════════════════════════════════════════════════════
            let coalesced_packets = coalesce_packets(outgoing_packets, self.config.max_coalesced_size);
        ...
        let coalesced_shutdown = coalesce_packets(shutdown_packets, self.config.max_coalesced_size);
