...
pub enum ThreadPriority {
    ...
    High,
    /// Maximum priority (real-time, requires privileges)
    Max,
}

/// CPU affinity strategy for thread placement
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum CpuAffinityStrategy {
    /// No CPU pinning (let OS scheduler decide)
    Auto,
    /// Interleaved: I/O on even cores, QUIC on odd cores
    /// Example: I/O(0,2,4,6), QUIC(1,3,5,7)
    Interleaved,
    /// Sequential: I/O on first N cores, QUIC on next N
    /// Example: I/O(0,1,2,3), QUIC(4,5,6,7)
    Sequential,
}
...
impl ConfigV2 {
    ...
    pub fn display_summary(&self) {
        ...
        log::info!("╔═══════════════════════════════════════════════════════════════╗");
        log::info!("║         superd - Production-Ready Architecture v2             ║");
        log::info!("╠═══════════════════════════════════════════════════════════════╣");
...
mod tests {
    ...
    fn test_io_thread_calculation() {
        assert_eq!(ConfigV2::calculate_io_threads(1), 1);
        assert_eq!(ConfigV2::calculate_io_threads(2), 1);
        assert_eq!(ConfigV2::calculate_io_threads(4), 1);
        assert_eq!(ConfigV2::calculate_io_threads(8), 2);
        assert_eq!(ConfigV2::calculate_io_threads(16), 4);
        assert_eq!(ConfigV2::calculate_io_threads(32), 8);
        assert_eq!(ConfigV2::calculate_io_threads(64), 8);
    }
