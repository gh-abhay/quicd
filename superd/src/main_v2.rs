...
/// Load configuration from TOML file
fn load_config_from_file(path: &PathBuf) -> Result<ConfigV2, Box<dyn std::error::Error>> {
...
fn apply_cli_overrides(config: &mut ConfigV2, cli: &Cli) -> Result<(), String> {
    // Server configuration
    config.server.listen_addr = cli.listen.parse()
...
fn parse_priority(s: &str) -> Result<ThreadPriority, String> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(format!("Invalid priority: {} (must be: low, normal, high, max)", s)),
    }
}
...
fn parse_affinity(s: &str) -> Result<CpuAffinityStrategy, String> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(format!("Invalid affinity: {} (must be: auto, interleaved, sequential)", s)),
    }
}
...
fn parse_cpu_mode(s: &str) -> Result<TokioCpuMode, String> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(format!("Invalid CPU mode: {} (must be: dedicated, shared)", s)),
    }
}
