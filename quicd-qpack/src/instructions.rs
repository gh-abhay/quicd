...
mod tests {
    ...
    #[test]
    fn test_set_capacity_encode_decode() {
    ...
    #[test]
    fn test_insert_with_name_ref_encode_decode() {
        let inst = EncoderInstruction::InsertWithNameRef {
            is_static: true,
            name_index: 17,
            value: Bytes::from("GET"),
            huffman_value: false,
        };
        let mut buf = Vec::new();
        inst.encode(&mut buf);
    ...
    #[test]
    fn test_insert_with_literal_name_encode_decode() {
        let inst = EncoderInstruction::InsertWithLiteralName {
            name: Bytes::from("custom-key"),
            huffman_name: false,
            value: Bytes::from("custom-value"),
            huffman_value: false,
        };
        let mut buf = Vec::new();
        inst.encode(&mut buf);
    ...
    #[test]
    fn test_duplicate_encode_decode() {
    ...
    #[test]
    fn test_section_ack_encode_decode() {
    ...
    #[test]
    fn test_stream_cancel_encode_decode() {
    ...
    #[test]
    fn test_insert_count_increment_encode_decode() {
    ...
    #[test]
    fn test_string_encoding_huffman() {
        let mut buf = Vec::new();
        encode_string(b"Hello", true, 8, 0, &mut buf);
        assert!(!buf.is_empty());
        assert!((buf[0] & 0x80) != 0); // Huffman bit set
    }

    #[test]
    fn test_string_encoding_literal() {
        let mut buf = Vec::new();
        encode_string(b"Hello", false, 8, 0, &mut buf);
        assert!(!buf.is_empty());
        assert_eq!((buf[0] & 0x80), 0); // Huffman bit not set
    }
}
